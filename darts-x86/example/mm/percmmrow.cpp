...
public:
	int i;
	Block(){}
	Block (uint32_t dep, uint32_t res, ThreadedProcedure * myTP, uint32_t st, int id);
	void initThis (uint32_t dep, uint32_t res, darts::ThreadedProcedure * myTP, uint32_t stat,
										int id);
...
public:
	...
	Codelet * toSignal;
	Master();
	~Master (){ delete [] blocks; delete [] prefA; delete [] prefB; };
};

Block::Block (uint32_t dep, uint32_t res, ThreadedProcedure * myTP, uint32_t st, int id):
...
Master::Master(): toSignal(finalSignal)
{
	...
	for(int i = 0; i < total_blocks; i++)
		blocks[i].initThis (2*num_blocks_p, 2*row_size, this, SHORTWAIT, i);
	for(int i = 0; i < total_blocks; i++)
	{
