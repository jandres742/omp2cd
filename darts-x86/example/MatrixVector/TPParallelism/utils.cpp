...
void openMPVersion (double *vec, double *mat, double *res, size_t matSize) {
	#pragma omp parallel for default (none) firstprivate(mat,vec,res) shared (matSize)
	for (unsigned i = 0; i < matSize; ++i) {
		double resTmp = 0.;
		for (unsigned j = 0; j < matSize; ++j) {
			resTmp += mat[i*matSize+j] * vec[j];
...
void dartsVersion (double *vec, double *mat, double *res, size_t size) {
	if (size < 3) return;
	Runtime().run(launch<MatrixVectorWrapper>(vec, mat, res, size, 4, 12, &Runtime::finalSignal));
}
