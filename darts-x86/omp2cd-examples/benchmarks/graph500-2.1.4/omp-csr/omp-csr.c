...
#include <stdlib.h>
#include <stdio.h>
struct timeval startTimeTotal, endTimeTotal;
struct timeval startTimeOuterWhile, endTimeOuterWhile;
long int timeOuterWhile = 0;
long int countOuterWhile = 0;
struct timeval startTimeWhile, endTimeWhile;
long int timeWhile = 0;
long int countWhile = 0;
struct timeval startTimeOMPFor, endTimeOMPFor;
long int timeOMPFor = 0;
long int countOMPFor = 0;
struct timeval startTimeFor, endTimeFor;
long int timeFor;
long int countFor;

/*
 * xadj[]: Has the adjancent vertex (neighbors) to each vertex. The neighbors for each
 * vertex are organized according to the offset in xoff[].
 * 
 * To find the neighbors for vertex k:
 * 1. Get the initial offset (including): XOFF(k) = xoff[2*(k)].
 * 2. Get the final offset (not including): XENDOFF(k) = xoff[1+2*(k)].
 * 3. The neighbors of vertex 3 are the values stored in [ xadj[XOFF(k)], xadj[XENDOFF(k)] ).
 * 
 * 
 */
...
static int setup_deg_off(const struct packed_edge *restrict IJ, int64_t nedge)
{
	...
	OMP("omp parallel") {
		...
		OMP("omp single") {
			...
			if (!
			    (xadjstore =
			     (int64_t *) xmalloc_large_ext((XOFF(nv) + MINVECT_SIZE) *
							   sizeof(*xadjstore))))
				err = -1;
			if (!err) {
...
{
	gettimeofday(&startTimeTotal, NULL);

	/*bfs_tree is the BFS output with the parent of each vertex:
	 * bfs[i] = j : j is the parent of i, when the root is srcvtx.
	 */
...
#define THREAD_BUF_LEN 16384
	...
	OMP("omp parallel shared(k1, k2)") {
		...
		int64_t *nbuf = (int64_t *) malloc(sizeof(int64_t) * THREAD_BUF_LEN);
		...
		if (omp_get_thread_num() == 0) {
			gettimeofday(&startTimeOuterWhile, NULL);
		}

		/* If at least one new vertex is visited, k2 will be updated. Otherwise,
		 * the while ends.
		 **/
		while (k1 != k2) {
			if (omp_get_thread_num() == 0) {
				gettimeofday(&startTimeWhile, NULL);
			}
			...
			if (omp_get_thread_num() == 0) {
				gettimeofday(&startTimeOMPFor, NULL);
			}

			/*Traverse the queue vlist from k1 to k2 concurrently. k2 - k1 + 1 = size of the frontier = num of vertices to check
			 * */
			OMP("omp for")
			    for (k = k1; k < oldk2; ++k) {
				if (omp_get_thread_num() == 0) {
					gettimeofday(&startTimeFor, NULL);
				}

				/*v is the current vertex whose neighbors will be visited. For them, v is its parent */
				const int64_t v = vlist[k];
				const int64_t veo = XENDOFF(v);
				int64_t vo;

				/*get the adjacent vertices for vertex v. The adjacent verticex are
				 *stored in xadj[] in the positions [XOFF(v), XENDOFF(v)).
				 * */
				for (vo = XOFF(v); vo < veo; ++vo) {
					/*get next adjacent vertex */
					const int64_t j = xadj[vo];

					/*Check if the parent of j is still undefined (undefined == -1) */
					if (bfs_tree[j] == -1) {

						/*assign v as parent of vertex j */
						if (int64_cas(&bfs_tree[j], -1, v)) {
							if (kbuf < THREAD_BUF_LEN) {
								/*store in nbuf the vertices just visited */
								nbuf[kbuf++] = j;
							} else {
								int64_t voff =
								    int64_fetch_add(&k2,
				...
				if (omp_get_thread_num() == 0) {
					gettimeofday(&endTimeFor, NULL);
					timeFor +=
					    ((endTimeFor.tv_sec * 1000000 + endTimeFor.tv_usec) -
					     (startTimeFor.tv_sec * 1000000 +
					      startTimeFor.tv_usec));
					countFor++;
				}
			}

			if (omp_get_thread_num() == 0) {
				gettimeofday(&endTimeOMPFor, NULL);
				timeOMPFor +=
				    ((endTimeOMPFor.tv_sec * 1000000 + endTimeOMPFor.tv_usec) -
				     (startTimeOMPFor.tv_sec * 1000000 + startTimeOMPFor.tv_usec));
				countOMPFor++;
			}

			/*kbuf > 0 if at least one new neighbor was visited */
			if (kbuf) {
				/*determine the tail of the queue vlist */
				int64_t voff = int64_fetch_add(&k2, kbuf), vk;

				/*assert (voff + kbuf <= nv); */

				/*append to the queue vlist the recently visited vertices */
				for (vk = 0; vk < kbuf; ++vk)
					vlist[voff + vk] = nbuf[vk];
			}
			OMP("omp single")
	...
	gettimeofday(&endTimeTotal, NULL);
...
int make_bfs_tree(int64_t * bfs_tree_out, int64_t * max_vtx_out, int64_t srcvtx)
{
	...
	int64_t *restrict vlist = (int64_t *) NULL;
	int64_t k1, k2;
