...
#if defined(_OPENMP)
static void randpermute_int64_t (int64_t *A_in, int64_t nelem,
      mrg_state * restrict st)
{
	...
	for (k = 0; k < nelem; ++k) {
		...
		if (k != place) {
			...
			release_i64 (&A[place], Aplace);
		}
		release_i64 (&A[k], Ak);
	}
}

static void randpermute_packed_edge (struct packed_edge *A_in, int64_t nelem,
      mrg_state * restrict st)
{
	struct packed_edge * restrict A = A_in;
	int64_t k;

	OMP("omp for")
	for (k = 0; k < nelem; ++k) {
		int64_t place;
		struct packed_edge Ak, Aplace;
		...
		if (k != place) {
			...
			Aplace = take_pe (&A[place]);

			{
				struct packed_edge t;
				t = A[place];
				A[place] = A[k];
				A[k] = t;
			}
...
static void randpermute_int64_t (int64_t *A_in, int64_t nelem, mrg_state * restrict st)
{
	int64_t * restrict A = A_in;
	int64_t k;
...
static void randpermute_packed_edge (struct packed_edge *A_in, int64_t nelem, mrg_state * restrict st)
{
	struct packed_edge * restrict A = A_in;
	int64_t k;
