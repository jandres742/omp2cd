...
/*Changing the name of 'array' to sort_array to avoid clashes with c++'s array when compiling
with a CXX compiler.*/
#if 0
ELM *array, *tmp;
#else
...
void sort_par ( void )
{
	...
	#pragma omp task untied
	     cilksort_par(sort_array, tmp, bots_arg_size);
