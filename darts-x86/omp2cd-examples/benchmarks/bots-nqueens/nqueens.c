...
#define MAX_SOLUTIONS sizeof(verify_solutions)/sizeof(int)
...
{
	fprintf(stderr, "verify_queens: size = %d, verify_solutions[%d] = %d\n", size, size-1, verify_solutions[size-1]);
	if ( size > (int)(MAX_SOLUTIONS) ) return BOTS_RESULT_NA;
	if ( total_count == verify_solutions[size-1]) return BOTS_RESULT_SUCCESSFUL;
