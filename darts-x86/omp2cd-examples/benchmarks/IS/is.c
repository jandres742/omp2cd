...
INT_TYPE key_array[SIZE_OF_BUFFERS],    
         ...
         **key_buff1_aptr = NULL;
#else
INT_TYPE *key_array, *key_buff1, *key_buff2, partial_verify_vals[TEST_ARRAY_SIZE],
    **key_buff1_aptr = NULL;
#endif
...
#else
INT_TYPE **bucket_size, bucket_ptrs[1 << 10]; /*Using max size from CLASS D*/
#endif
...
int main(int argc, char** argv)
{
    ...
    if (passed_verification != 5 * MAX_ITERATIONS + 1)
        passed_verification = 0;
    c_print_results("IS", CLASS, (int)(TOTAL_KEYS / 64), 64, 0, MAX_ITERATIONS, timecounter,
        ((double)(MAX_ITERATIONS * TOTAL_KEYS)) / timecounter / 1000000., "keys ranked",
        passed_verification, NPBVERSION, COMPILETIME, CC, CLINK, C_LIB, C_INC, CFLAGS, CLINKFLAGS);
