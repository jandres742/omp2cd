...
using namespace std;
long long LARGE_NUM = 5764607523034234879LL;
long UpPrimeLim = 100000;
long adcexp[20] = { 11, 13, 17, 19, 23, 23, 29, 31, 37, 41, 41, 43, 47, 53, 59, 3, 5, 7, 11, 13 };
long adcexpA[15] = { 2 * 2, 2 * 2 * 2 * 5, 2 * 3, 2 * 2 * 5, 2 * 3 * 7, 2 * 19, 2 * 13, 2 * 19,
    2 * 2 * 2 * 13 * 19, 2 * 2 * 2 * 19 * 19, 2 * 23, 2 * 2 * 2 * 2, 2 * 2 * 2 * 2 * 2 * 23,
    2 * 2 * 2 * 2 * 2, 2 * 2 * 23 };
long adcexpB[20] = { 2 * 2 * 7, 2 * 2 * 2 * 5, 2 * 3 * 7, 2 * 2 * 5 * 7, 2 * 3 * 7 * 7, 2 * 19,
    2 * 13, 2 * 19, 2 * 2 * 2 * 13 * 19, 2 * 2 * 2 * 19 * 19, 2 * 31, 2 * 2 * 2 * 2 * 31,
    2 * 2 * 2 * 2 * 2 * 31, 2 * 2 * 2 * 2 * 2 * 29, 2 * 2 * 29, 2 * 43, 2 * 2, 2 * 2, 2 * 2 * 47,
    2 * 2 * 2 * 43 };
long adcexpS[5] = { 11, 13, 17, 19, 23 };
long adcexpW[10]
    = { 2 * 2, 2 * 2 * 2 * 5, 2 * 3, 2 * 2 * 5, 2 * 3 * 7, 23, 29, 31, 2 * 2, 2 * 2 * 19 };
long adcgen[20] = { 2, 7, 3, 2, 2, 2, 2, 5, 31, 7, 2, 3, 3, 3, 2, 5, 2, 2, 2, 3 };
long adcprime[20] = { 421, 601, 631, 701, 883, 419, 443, 647, 21737, 31769, 1427, 18353, 22817,
    34337, 98717, 3527, 8693, 9677, 11093, 18233 };
long long maxprmfctr = 59;
int measbound_darts17;
bool measbound_darts17_initFlag;
long seed_darts17[20];
int tuplenum_darts17;
bool tuplenum_darts17_initFlag;
int maxdim_darts17;
bool maxdim_darts17_initFlag;
/*Function: swap4, ID: 1*/
void swap4(void* num)
{
    /*swap4:1*/
...
/*Function: ShowFactorization, ID: 6*/
...
long ListFirstPrimes(long mpr, long* prlist)
{
    ...
    for (nmb = 8; nmb < mpr; nmb++) {
        ...
        for (j = 0; prlist[j] * prlist[j] <= nmb; j++) {
            if (nmb - prlist[j] * ((long)(nmb / prlist[j])) == 0) {
                composed = 1;
                break;
            }
        }
...
/*Function: GetFactorization, ID: 10*/
void GetFactorization(long prnum, long* prlist, Factorization** fctlist)
{
    ...
    long len = 0, isft = 0, div = 1, firstdiv = 1;
    fct->dim = 2;
    fct->mlt = (long*)malloc(2 * sizeof(long));
    fct->exp = (long*)malloc(2 * sizeof(long));
    for (i = 0; i < fct->dim; i++) {
        fct->mlt[i] = 1;
        fct->exp[i] = 0;
    }
    ...
    for (i = 0; i < prlist[prnum - 1]; i++) {
        ...
        if (len == 0)
            len = 1;
        fct->mlt = (long*)malloc(len * sizeof(long));
        fct->exp = (long*)malloc(len * sizeof(long));
        for (j = 0; j < fct->dim; j++) {
            fct->mlt[j] = 1;
            fct->exp[j] = 0;
        }
        ...
        for (j = 0; prlist[j] * prlist[j] <= i; j++) {
            firstdiv = prlist[j];
            if (i - firstdiv * ((long)i / firstdiv) == 0) {
                div = firstdiv;
...
/*Function: CompareSizesByValue, ID: 11*/
int CompareSizesByValue(const void* sz0, const void* sz1)
{
    /*CompareSizesByValue:11*/
    /*CompoundStmt:4945*/
...
/*Function: CompareViewsBySize, ID: 12*/
int CompareViewsBySize(const void* vw0, const void* vw1)
{
    /*CompareViewsBySize:12*/
    /*CompoundStmt:4961*/
...
/*Function: CalculateVeiwSizes, ID: 13*/
int CalculateVeiwSizes(ADC_PAR* par)
{
    /*CalculateVeiwSizes:13*/
    /*CompoundStmt:4988*/
    ...
    char clss = par->clss;
    int dcdim = par->dim;
    long long tnum = par->tuplenum;
    long long i = 0, j = 0;
    Factorization** fctlist = (Factorization**)calloc(UpPrimeLim, sizeof(Factorization*));
    long* prlist = (long*)calloc(UpPrimeLim, sizeof(long));
        ...
        while (kwd) {
            if (strstr(line, "#")) {
                ...
                case 5:
                    ...
                    sscanf(pos, "%s", (char*)par->filename);
                    break;
                case 6:
...
int GetNextTuple(int dcdim, int measnum, long long* attr, long long* meas, char clss)
{
    ...
    for (i = 0; i < dcdim; i++) {
        ...
        attr[i] -= adcprime[i] * ((long long)attr[i] / adcprime[i]);
        seed[i] = attr[i];
        if (seed[i] > maxattr)
            maxattr = seed[i];
    }
    for (i = 0; i < measnum; i++) {
        meas[i] = (long long)(seed[i] * maxattr);
        meas[i] -= measbound * (meas[i] / measbound);
    }
    return 1;
}
/*Function: GenerateADC, ID: 5*/
int GenerateADC(ADC_PAR* par)
{
    ...
    FILE* adc;
    int i = 0, j = 0;
    long long *attr = ((void *)0), *mes = ((void *)0);
    sprintf(adcfname, "%s.dat.%d", par->filename, par->ndid);
    if (!(adc = fopen(adcfname, "wb+"))) {
        fprintf(stderr, "GenerateADC: Can't open file: %s\n", adcfname);
        return 0;
    }
    attr = (long long*)malloc(dcdim * sizeof(long long));
    mes = (long long*)malloc(mesnum * sizeof(long long));
