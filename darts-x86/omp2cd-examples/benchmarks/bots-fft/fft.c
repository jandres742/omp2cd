...
void fft_twiddle_2(int a, int b, COMPLEX* in, COMPLEX* out, COMPLEX* W, int nW, int nWdn, int m)
{
    ...
    if ((b - a) < 128) {
/*Changing the form of this loop according to omp2cd's current support*/
#if 1
        for (i = a; i < b; i++) {
            l1 = nWdn * i;
            kp = out + i;
...
int test_correctness(int n, COMPLEX* out1, COMPLEX* out2)
{
    ...
    for (i = 0; i < n; ++i) {
        a = sqrt((c_re(out1[i]) - c_re(out2[i])) * (c_re(out1[i]) - c_re(out2[i])) + (c_im(out1[i]) - c_im(out2[i])) * (c_im(out1[i]) - c_im(out2[i])));
