...
{
    if (this->getID() == 0) {
        invoke<TP272>(myTP, 1, this->getID(), myTP);
    } else {
...
void TP272::_checkInCodelets274::fire(void)
{
    ...
    myTP->controlTPParent->TPParent->TPParent->barrierCodelets270[0].decDep();
}
TP272::TP272(int in_numThreads, int in_mainCodeletID, TP270* in_TPParent)
    : ompOMPMasterDirectiveTP(in_numThreads, in_mainCodeletID)
    , TPParent(in_TPParent)
    , controlTPParent(this)
    , inputsTPParent(this)
    , checkInCodelets274(1, 1, this, this->mainCodeletID)
{
    /*Initialize inputs and vars.*/
    /*Initialize Codelets*/
    checkInCodelets274.decDep();
}
TP272::~TP272() {}
