...
void usage(char* argv[])
{
    printf("Usage: %s.x \n"
        ...
        argv[0], DEFAULT_OUTER_REPS, DEFAULT_TEST_TARGET_TIME,
        DEFAULT_DELAY_TIME);
}
...
unsigned long getinnerreps(void (*test)(void))
{
    ...
    while (time < targettesttime) {
        ...
        // Test to stop code if compiler is optimising reference time
        // expressions away
...
void stats(double* mtp, double* sdp)
{
    ...
    printf("Sample_size       Average     Min         Max          S.D.        "
           "  Outliers\n");
    printf(" %d                %f   %f   %f    %f      %d\n", outerreps,
        meantime, mintime, maxtime, sd, nr);
    printf("\n");
    ...
    double referencetime, double refsd)
{
    ...
    printf("%s overhead = %f microseconds +/- %f\n", name,
        testtime - referencetime, CONF95 * (testsd + referencesd));
}
...
#pragma omp master
    ...
    delaylength = getdelaylengthfromtime(
        delaytime); // Always need to compute delaylength in iterations
...
void finalise(void) { free(times); }

void initreference(char* name) { printheader(name); }
...
void intitest(char* name) { printheader(name); }
