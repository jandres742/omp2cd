    ...
    for (ii = 0; ii < bots_arg_size; ii++) {
        for (jj = 0; jj < bots_arg_size; jj++) {
            ...
            if (null_entry == FALSE) {
                ...
                if (M[ii * bots_arg_size + jj] == (float*)NULL) {
                ...
                M[ii * bots_arg_size + jj] = (float*)NULL;
            }
        }
    }
}
...
#pragma omp task untied
    for (kk = 0; kk < bots_arg_size; kk++) {
        ...
        for (jj = kk + 1; jj < bots_arg_size; jj++)
            if (BENCH[kk * bots_arg_size + jj] != (float*)NULL)
#pragma omp task untied firstprivate(kk, jj) shared(BENCH)
            {
                fwd(BENCH[kk * bots_arg_size + kk], BENCH[kk * bots_arg_size + jj]);
            }
        for (ii = kk + 1; ii < bots_arg_size; ii++)
            if (BENCH[ii * bots_arg_size + kk] != (float*)NULL)
...
#pragma omp task untied firstprivate(kk, jj, ii) shared(BENCH)
                    {
                        if (BENCH[ii * bots_arg_size + jj] == (float*)NULL)
