...
void getOMPSchedulePolicy()
{
    ...
    if (ompSchedule != NULL) {
        ...
        if (pos != string::npos) {
            string tempChunk
                = ompSchedStr.substr(pos + 1, (ompSchedStr.length() - pos));
...
size_t codeletsPerTP(size_t numThreads, size_t startCodeletID)
{
    ...
    if (numThreads % getNumOfPUs() > 0
        && startCodeletID == getNumOfTPChildren(numThreads) - 1)
        res = numThreads % getNumOfPUs();
    return res;
}

std::vector<mutex> mutexFunctionStack(MAXNUMTHREADS);
std::vector<std::vector<void*> > threadFunctionStack(MAXNUMTHREADS);
void pushFunctionStack(uint32_t codeletID, void* funcPtr)
{
    mutexFunctionStack[codeletID].lock();
    threadFunctionStack[codeletID].push_back(funcPtr);
    mutexFunctionStack[codeletID].unlock();
}

void popFunctionStack(uint32_t codeletID)
{
    mutexFunctionStack[codeletID].lock();
    threadFunctionStack[codeletID].pop_back();
    mutexFunctionStack[codeletID].unlock();
}
...
void* topFunctionStack(uint32_t codeletID)
{
    void* ptr = NULL;
    mutexFunctionStack[codeletID].lock();
    if (threadFunctionStack[codeletID].size() > 0)
        threadFunctionStack[codeletID].back();
    mutexFunctionStack[codeletID].unlock();
    return ptr;
}
