...
void DeclPrinter::VisitFunctionDecl(FunctionDecl *D) {
  ...
  if(D->getBackEnd().compare("swarm") == 0){
	  Proto += "_swarm";
  }
  ...
  if (const FunctionType *AFT = Ty->getAs<FunctionType>()) {
	...
	if(D->getBackEnd().compare("swarm") == 0){
		Proto += ", uint64_t codeletID, swarm_Codelet_t *nextCodelet, void *nextCodeletContext";
	}
  ...
  if (D->isPure())
    ...
    if (D->getBody() && D->getPrintBodyFlag())
      D->getBody()->printPretty(Out, nullptr, SubPolicy, Indentation);
  
	if(D->getPrintBodyFlag())
	  Out << '\n';
  }
}
