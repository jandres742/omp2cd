...
public:
  ...
  /// \brief Marks current region as codelet (it has a 'codelet' clause).
  void setCodeletRegion(bool IsCodelet = true) {
    Stack.back().CodeletRegion = IsCodelet;
  }
  ...
  /// \brief Returns true, if parent region is codelet (has associated
  /// 'codelet' clause), false - otherwise.
  bool isParentCodeletRegion() const {
    if (Stack.size() > 2)
      return Stack[Stack.size() - 2].CodeletRegion;
    return false;
  }
...
StmtResult Sema::ActOnOpenMPSingleDirective(ArrayRef<OMPClause *> Clauses,
  ...
  OMPClause *Codelet = nullptr;
  (void)Codelet;
  OMPClause *Copyprivate = nullptr;
  for (auto *Clause : Clauses) {
    if (Clause->getClauseKind() == OMPC_nowait)
      Nowait = Clause;
    else if (Clause->getClauseKind() == OMPC_codelet)
      Codelet = Clause;
...
OMPClause *Sema::ActOnOpenMPClause(OpenMPClauseKind Kind,
  ...
  case OMPC_nowait:
    ...
    break;
  case OMPC_codelet:
    Res = ActOnOpenMPCodeletClause(StartLoc, EndLoc);
    break;
  case OMPC_untied:
...
OMPClause *Sema::ActOnOpenMPCodeletClause(SourceLocation StartLoc,
                                         SourceLocation EndLoc) {
  DSAStack->setCodeletRegion();
  return new (Context) OMPCodeletClause(StartLoc, EndLoc);
}
